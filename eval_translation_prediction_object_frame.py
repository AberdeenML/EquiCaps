...
class Dataset3DIEBenchT(Dataset):
    ...
    def __getitem__(self, i):
        ...
        latent_1 = np.load(str(self.dataset_root)  + self.samples[i]+ f"/latent_{views[0]}.npy")[-3:].astype(np.float32)
        latent_2 = np.load(str(self.dataset_root)  + self.samples[i]+ f"/latent_{views[1]}.npy")[-3:].astype(np.float32)
