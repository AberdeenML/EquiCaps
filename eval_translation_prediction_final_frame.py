...
class Dataset3DIEBenchT(Dataset):
    ...
    def __getitem__(self, i):
        ...
        latent_1 = np.load(str(self.dataset_root)  + self.samples[i]+ f"/latent_{views[0]}.npy").astype(np.float32)
        latent_2 = np.load(str(self.dataset_root)  + self.samples[i]+ f"/latent_{views[1]}.npy").astype(np.float32)
        ...
        translation_1 = M1[:3,3]
        translation_2 = M2[:3,3]

        translation = translation_2  - translation_1 

        return img_1, img_2, torch.FloatTensor(translation)
...
### DATA
ds_train = Dataset3DIEBenchT(args.dataset_root,
...
ds_val = Dataset3DIEBenchT(args.dataset_root,
                        "./data/val_images.npy",
                        transform=transforms.Compose([transforms.Resize((args.resolution,args.resolution)),transforms.ToTensor(),normalize]))
